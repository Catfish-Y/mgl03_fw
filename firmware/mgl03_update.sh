...
FW_URI_LIST=$(curl -s -k -o- "https://api.kgithub.com/repos/zvldz/mgl03_fw/git/trees/main?recursive=1" | grep custom | grep mod | grep zip | sort | cut -f4 -d'"')
FW_URI_LIST_STOCK=$(curl -s -k -o- "https://api.kgithub.com/repos/zvldz/mgl03_fw/git/trees/main?recursive=1" | grep stock | grep zip | sort | cut -f4 -d'"')
...
FW_URL="https://raw.yzuu.cf/zvldz/mgl03_fw/main/${FW_URI}"
